...
const RoomEditCard = ({ roomData }) => {
    ...
    const [editedRoomType, setEditedRoomType] = useState(roomData.room_type || 'Room Type 1');
    const [editedPrice, setEditedPrice] = useState(roomData.price_per_night || 'MK200 / Night');
    const [editedDescription, setEditedDescription] = useState(roomData.brief_description_of_room || 'Description');
    const [editedFromDate, setEditedFromDate] = useState(roomData.availability_dates ? roomData.availability_dates.from : '');
    const [editedToDate, setEditedToDate] = useState(roomData.availability_dates ? roomData.availability_dates.to : '');
    ...
    const handleSaveClick = async () => {
        const roomDocRef = doc(db, 'rooms', roomData.id);

        // Update the document with the edited data
        await setDoc(roomDocRef, {
            room_type: editedRoomType,
            brief_description_of_room: editedDescription,
            availability_dates: {
                from: editedFromDate,
                to: editedToDate,
            },
            price_per_night: editedPrice,
    ...
    const handleCancelClick = () => {
        ...
        setIsEditing(false);
        setEditedRoomType(roomData.room_type || 'Room Type 1');
        setEditedDescription(roomData.brief_description_of_room || "Description")
        setEditedFromDate(roomData.availability_dates ? roomData.availability_dates.from : '');
        setEditedToDate(roomData.availability_dates ? roomData.availability_dates.to : '');
        setEditedPrice(roomData.price_per_night || 'MK200 / Night');
    };
    ...
    return (
        <div className='md:w-[450px] w-[290px] flex md:flex-row flex-col gap-3 justify-center items-center'>
            <div className='w-[270px] h-[200px] relative'>
                <img className='w-[270px] h-[200px]' src={roomData.photos[0] || c1} alt="" />
            ...
            <div className='flex flex-col gap-3 justify-center items-center'>
                ...
                {/* <p className='text-black'>{truncateText(roomData.brief_description_of_room, 35)}</p> */}
                ...
                <div className='w-full flex flex-col justify-between items-start'>
                    <p className='text-[20] text-black font-semibold'>
                        {isEditing ? (
                            ...
                            truncateText(editedPrice, 9) + "MK / Night"
                        )}
                    </p>
                    {isEditing ? (
                        <>
