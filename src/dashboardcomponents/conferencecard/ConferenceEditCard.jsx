...
const ConferenceEditCard = ({ conferenceData }) => {
    ...
    const [editedRoomType, setEditedRoomType] = useState(conferenceData.name || 'NULL');
    const [editedDescription, setEditedDescription] = useState(conferenceData.brief_description_of_venue || 'NULL');
    // const [editedPrice, setEditedPrice] = useState(conferenceData.price || 'NaN');
    const [editedFromDate, setEditedFromDate] = useState(conferenceData.availability_dates ? conferenceData.availability_dates.from : 'NULL');
    const [editedToDate, setEditedToDate] = useState(conferenceData.availability_dates ? conferenceData.availability_dates.to : 'NULL');
    ...
    const handleSaveClick = async () => {
        const roomDocRef = doc(db, 'conferences', conferenceData.id);
    ...
    const handleCancelClick = () => {
        ...
        setIsEditing(false);
        setEditedRoomType(conferenceData.name || 'NULL');
        setEditedDescription(conferenceData.brief_description_of_venue || "NULL")
        setEditedFromDate(conferenceData.availability_dates ? conferenceData.availability_dates.from : 'NULL');
        setEditedToDate(conferenceData.availability_dates ? conferenceData.availability_dates.to : 'NULL');
        // setEditedPrice(conferenceData.price || 'NaN');
    };
    ...
    return (
        <div className='md:w-[450px] w-[290px] flex md:flex-row flex-col gap-3 justify-center items-center'>
            <div className='w-[270px] h-[200px] relative'>
                <img className='w-[270px] h-[200px]' src={conferenceData.photos[0]?conferenceData.photos[0]: c1} alt="" />
            ...
            <div className='flex flex-col gap-3 justify-center items-center'>
                <div className='w-full flex flex-row justify-between items-center'>
                    <p className='font-medium text-[17px] text-black'>
                        {isEditing ? (
                            ...
                            truncateText(editedRoomType, 25)
                        )}
                    </p>
                ...
                {/* <p className='text-black'>{truncateText(conferenceData.brief_description_of_venue, 35)}</p> */}
...
export default ConferenceEditCard;
